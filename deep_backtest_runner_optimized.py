...
STRATEGY_CONFIG['TRENDING']['TP_RR'] = 1.5
...
def simulate_trend_logic_15m(df_15m):
    ...
    for i in range(50, len(df_15m) - 10):
        ...
        if long_score >= STRATEGY_CONFIG['TRENDING']['MIN_SCORE']:
            ...
            tp_price = entry_price + (m15['atr'] * (STRATEGY_CONFIG['TRENDING']['SL_ATR_MULT'] * STRATEGY_CONFIG['TRENDING']['TP_RR'])) # RR 1.5:1
...
def simulate_momentum_logic_3m(df_3m):
    ...
    for i in range(50, len(df_3m) - 10):
        ...
        if long_score >= STRATEGY_CONFIG['TRENDING']['MIN_SCORE']:
            ...
            tp_price = entry_price + (m3['atr'] * (STRATEGY_CONFIG['MOMENTUM']['SL_ATR_MULT'] * STRATEGY_CONFIG['MOMENTUM']['TP_RR']))
            ...
            for j in range(i+1, min(i+120, len(df_3m))):
                ...
                if future_bar['low'] <= sl_price:
                    ...
                    trade_result = "WIN"
                    break
            trades.append({'time': m3['timestamp'], 'filled': fill_status, 'result': trade_result, 'rr': STRATEGY_CONFIG['MOMENTUM']['TP_RR']})
    return trades
...
def simulate_scalping_logic_3m(df_3m):
    ...
    for i in range(150, len(df_3m) - 10):
        ...
        if long_score >= STRATEGY_CONFIG['TRENDING']['MIN_SCORE']:
            ...
            tp_price = entry_price + (m3['atr'] * STRATEGY_CONFIG['SCALPING']['SL_ATR_MULT'])
            fill_status, trade_result = True, "PENDING"
            for j in range(i+1, min(i+40, len(df_3m))): # Lookahead 2 hours
                ...
                if future_bar['low'] <= sl_price:
                    ...
                    trade_result = "WIN"
                    break
            trades.append({'time': m3['timestamp'], 'filled': fill_status, 'result': trade_result, 'rr': STRATEGY_CONFIG['SCALPING']['TP_RR']})
    return trades
...
def simulate_sideways_logic_3m(df_3m):
    ...
    for i in range(50, len(df_3m) - 10):
        ...
        if (m3['rsi'] < 45 and m3['low'] <= m3['bb_lower'] and m3['adx'] < 28 and long_trend_ok):
            entry_price = m3['close']
            sl_price = entry_price - (m3['atr'] * (STRATEGY_CONFIG['MOMENTUM']['SL_ATR_MULT'] * STRATEGY_CONFIG['MOMENTUM']['TP_RR']))
            tp_price = m3['bb_mid']
