...
# --- PROPOSED OPTIMIZATIONS FOR TEST ---
STRATEGY_CONFIG['TRENDING']['TP_RR'] = 1.2
STRATEGY_CONFIG['TRENDING']['MIN_SCORE'] = 70
STRATEGY_CONFIG['TRENDING']['FILTERS']['MIN_ATR_PCT'] = 0.25

STRATEGY_CONFIG['SIDEWAYS']['MIN_SCORE'] = 70
STRATEGY_CONFIG['SCALPING']['MIN_SCORE'] = 60
# ----------------------------------------
...
def simulate_trend_logic_15m(df_15m):
    ...
    for i in range(50, len(df_15m) - 10):
        ...
        if m15['adx'] > STRATEGY_CONFIG['TRENDING']['FILTERS']['MIN_ADX']: long_score += 15
        
        if long_score >= STRATEGY_CONFIG['TRENDING']['MIN_SCORE']:
            # Pullback entry at EMA7
            entry_price = m15['ema25']
            original_sl = entry_price - (m15['atr'] * STRATEGY_CONFIG['TRENDING']['SL_ATR_MULT'])
            sl_price = original_sl
            tp_price = entry_price + (m15['atr'] * (STRATEGY_CONFIG['TRENDING']['SL_ATR_MULT'] * 1.2)) # RR 1.5:1
            ...
            for j in range(i+1, min(i+40, len(df_15m))): # Lookahead 10 hours
                ...
                if fill_status:
                    ...
                    # Check TP
                        ...
                        trade_result = "WIN"
                        actual_rr = STRATEGY_CONFIG['TRENDING']['TP_RR']
                        break
...
def simulate_momentum_logic_3m(df_3m):
    ...
    for i in range(50, len(df_3m) - 10):
        ...
        if not volume_high or m3['adx'] < STRATEGY_CONFIG['MOMENTUM']['FILTERS']['MIN_ADX']: continue
        ...
        if (m3['adx'] >= 30 and m3['ema7'] > m3['ema25'] and m3['rsi'] >= STRATEGY_CONFIG['MOMENTUM']['FILTERS']['RSI_MIN_LONG']):
            entry_price = m3['ema7']
            sl_price = entry_price - (m3['atr'] * STRATEGY_CONFIG['MOMENTUM']['SL_ATR_MULT'])
            risk = entry_price - sl_price
            tp_price = entry_price + (risk * STRATEGY_CONFIG['MOMENTUM']['TP_RR'])
            
            # Simulate Fill
            if m3['low'] <= entry_price:
                ...
                for j in range(i+1, min(i+40, len(df_3m))): # Lookahead 2 hours
                    ...
                    if future_bar['low'] <= sl_price:
                        ...
                        trade_result = "WIN"
                        break
                actual_rr = STRATEGY_CONFIG['MOMENTUM']['TP_RR'] if trade_result == "WIN" else -1.0
                trades.append({'time': m3['timestamp'], 'filled': fill_status, 'result': trade_result, 'rr': actual_rr})
    return trades
...
def simulate_scalping_logic_3m(df_3m):
    ...
    for i in range(150, len(df_3m) - 10):
        ...
        if long_score >= STRATEGY_CONFIG['SCALPING']['MIN_SCORE']:
            entry_price = m3['ema25'] # Limit order pullback
            sl_price = entry_price - (m3['atr'] * STRATEGY_CONFIG['SCALPING']['SL_ATR_MULT'])
            risk = entry_price - sl_price
            tp_price = entry_price + (risk * STRATEGY_CONFIG['SCALPING']['TP_RR'])
            ...
            if m3['low'] <= entry_price:
                ...
                for j in range(i+1, min(i+40, len(df_3m))): # Lookahead 2 hours
                    ...
                    if future_bar['low'] <= sl_price:
                        ...
                        trade_result = "WIN"
                        break
                actual_rr = STRATEGY_CONFIG['SCALPING']['TP_RR'] if trade_result == "WIN" else -1.0
                trades.append({'time': m3['timestamp'], 'filled': fill_status, 'result': trade_result, 'rr': actual_rr})
    return trades
...
def simulate_sideways_logic_3m(df_3m):
    ...
    for i in range(50, len(df_3m) - 10):
        ...
        if (m3['rsi'] < 45 and m3['low'] <= m3['bb_lower'] and m3['adx'] <= STRATEGY_CONFIG['SIDEWAYS']['FILTERS']['MAX_ADX'] and long_trend_ok):
            entry_price = m3['close']
            sl_price = entry_price - (m3['atr'] * STRATEGY_CONFIG['SIDEWAYS']['SL_ATR_MULT'])
            tp_price = m3['bb_mid']
            ...
            for j in range(i+1, min(i+40, len(df_3m))):
                ...
                if future_bar['low'] <= sl_price:
                    ...
                    trade_result = "WIN"
                    break
            actual_rr = expected_rr if trade_result == "WIN" else -1.0
            trades.append({'time': m3['timestamp'], 'filled': fill_status, 'result': trade_result, 'rr': actual_rr})
    return trades
