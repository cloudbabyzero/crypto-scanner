...
def get_market_regime(btc_1h_bar):
    ...
    # Momentum/Volatile -> TREND mode logic now
    ...
    if btc_adx < 20 and btc_atr_percent < 0.15: return "SIDEWAYS"
    if btc_adx < 25: return "SIDEWAYS"
    return "SIDEWAYS"
...
def simulate_trend(df_15m, btc_df_4h, btc_df_1h):
    trades = []
    for i in range(100, len(df_15m) - 20):
        m15 = df_15m.iloc[i]
        btc_1h = btc_df_1h[btc_df_1h['timestamp'] <= m15['timestamp']].iloc[-1]
        ...
        btc_trend = _get_btc_trend(btc_df_4h, m15['timestamp'])
        if btc_trend == "bullish": short_score = 0
        elif btc_trend == "bearish": long_score = 0
        elif btc_trend == "neutral": long_score -= 20; short_score -= 20
...
def simulate_scalping(df_3m, df_15m, btc_df_4h, btc_df_1h):
    trades = []
    for i in range(100, len(df_3m) - 20):
        ...
        btc_trend = _get_btc_trend(btc_df_4h, m3['timestamp'])
        if btc_trend == "bullish": short_score = 0
        elif btc_trend == "bearish": long_score = 0
        elif btc_trend == "neutral": long_score -= 20; short_score -= 20
        long_score = min(long_score, 100); short_score = min(short_score, 100)
...
def simulate_sideways(df_3m, btc_df_4h, btc_df_1h):
    trades = []
    for i in range(100, len(df_3m) - 20):
        m3 = df_3m.iloc[i]
        btc_1h = btc_df_1h[btc_df_1h['timestamp'] <= m3['timestamp']].iloc[-1]
        ...
        elif btc_trend == "neutral": score -= 20
        ...
        if grade in ["A", "A+"]:
            ...
            fill_status, result = True, "PENDING"
...
def print_summary(test_name, all_trades):
    ...
    print("\n--- Breakdown by Mode ---")
    modes = ['TREND', 'SCALPING', 'SIDEWAYS']
    for m in modes:
        m_trades = [t for t in filled_trades if t['mode'] == m]
        if not m_trades: continue
...
if __name__ == "__main__":
    print(f"Fetching data for Auto-Routing A/B Test...")
    ...
    print("\nRunning Auto-Routing Test (MOMENTUM falls back to TREND)...")
    t_trend = simulate_trend(df_15m, btc_df_4h, btc_df_1h)
    t_scalp = simulate_scalping(df_3m, df_15m, btc_df_4h, btc_df_1h)
    t_side = simulate_sideways(df_3m, btc_df_4h, btc_df_1h)
    
    all_auto = t_trend + t_scalp + t_side
    
    print_summary("TEST: AUTO-ROUTING WITH BTC FILTER & DISABLED MOMENTUM", all_auto)
